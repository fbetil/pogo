...
{strip}
	...
	{if !$project->isNew()}
		...
		<section id="files" class="mts mbm">
			...
			{foreach from=$files item=file}
				...
				{if $file@last}</ul>{/if}
			{foreachelse}
				<p class="strong{if isset($user_roles['FileEditor'])} dropzone{/if}" data-folder="" >{lang("project_view_p_7")}</p>
				<ul class="unstyled">
					<li class="small"><i>{lang('project_view_p_1')}</i></li>
				</ul>
			{/foreach}
		...
		<!-- Project actors -->
		{if isset($user_roles['ProjectActorViewer'])}
		<section id="actors" class="mts mbm">
			<p class="h5-like"><i class="mrs fam fam_user_gray"></i>{lang('project_view_h_4')}
				{if isset($user_roles['ProjectActorEditor'])}<a class="right mrs" href="{$url_index}/projectactor/add/{$project->getId()}" title="{lang('project_view_a_4')}"><i class="fam fam_user_add"></i></a>{/if}
			</p>
			<ul class="unstyled">
			{foreach from=$project->getProjectActors() item=projectactor}
				<li class="pointer list" onclick="window.location.href = '{$url_index}/projectactor/view/{$projectactor->getId()}'">
					{$projectactor->getActor()->getFirstName()} {$projectactor->getActor()->getName()} - <i>{$projectactor->getRole()}</i>
					<div class="inbl list_actions right">
						{if isset($user_roles['ProjectActorEditor'])}<a href="javascript:void(0)" onclick="ajax('/projectactor/delete/{$projectactor->getId()}');" title="{lang('project_view_a_7')}"><i class="fam fam_user_delete"></i></a>{/if}
					</div>
				</li>
			{foreachelse}
				<li class="small"><i>{lang('project_view_p_1')}</i></li>
	...
	{else}
		<script>
			/*Start eip*/
			$(document).ready(function() { eipStart('project', '{$url_index}/project/view/$id', '{$url_index}/project'); });
		</script>
	{/if}
